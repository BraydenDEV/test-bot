...
exports.run = async(client, message, ops) => {
    ...
    if (message.guild.channels.find(channel => channel.name === `Report-${id.slice(14)}`)) return message.author.send('❌ You have a Ticket open ❌');
    message.guild.createChannel(`Report-${id.slice(14)}`, "text").then(c => {
        ...
        c.overwritePermissions(message.author, {
            ...
            message.channel.send(`:white_check_mark: Report created! Name: #report-${id.slice(14)}`);
        let newTicketEmbed = new Discord.RichEmbed()
            .setTitle(`Report Created By ${message.author.username}`)
            .setColor('0x0099ff')
            .setDescription(`Topic: **${topic}**`)
            .addField(`Hello ${message.author.tag}`, `How can we help you?`) 
            .addField(`Close the Report?`, '-close to close The Ticket')
            .setTimestamp();
